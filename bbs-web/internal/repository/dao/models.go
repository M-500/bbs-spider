...
type UserMode struct {
	gorm.Model
	Username string    `gorm:"uniqueIndex;not null;varchar(64);column:username;comment:用户名，唯一索引，不能为空"`
	Phone    string    `gorm:"index;varchar(32);comment:手机号;column:phone"`
	Email    string    `gorm:"index;varchar(128);comment:邮箱;column:email"`
	Password string    `gorm:"index;varchar(128);comment:密码;column:password"`
	Nickname string    `gorm:"index;varchar(128);comment:昵称;column:nickname"`
	Gender   string    `gorm:"index;varchar(128);comment:性别;column:gender"`
	Birthday time.Time `gorm:"index;comment:生日;column:birthday"`
